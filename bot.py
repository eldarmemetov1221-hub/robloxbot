...
GAMEPASS_REASONS = [
    ...
    (
        "Никнейм не найден",
        "❌ Транзакция не выполнена! \n"
        "Причина: Не найден аккаунт с таким никнейм ( Нет однозначного совпадения ) \n\n"
        "Проверьте никнейм (  Будьте внимательны указывать нужно юзернейм начинающийся с \"@ Он индивидуальный ) "
        "и попробуйте активировать код еще раз\n\n"
        "Или Обратитесь к менеджеру - @vallmanager или @vallmanager1 \n"
        "Для уточнения и решения.",
    ),
]
...
def roblox_get_gamepasses(user_id):
    """Список гейм-пассов пользователя (apis endpoint): [{id, name, price}].

    Видит пассы и на приватных играх. None -> все попытки запроса упали.
    """
    result = []
    cur = None
    while True:
        ...
        # до 3 попыток (эндпоинт бывает флаковый / отдаёт 429)
        payload = None
        ...
        if payload is None:
            return None  # не смогли получить страницу
...
def build_gamepass_report(nickname, expected_price=None, robux_amount=None):
    ...
    for g in games:
        ...
        if not universe_id:
            continue
        for gp in (roblox_get_universe_passes(universe_id) or []):
            gid = gp.get("id")
            if gid is None:
                continue
    ...
    # Уточняем цену/статус продажи там, где цена неизвестна (managed pricing и т.п.)
